...
--[MagiskPolicy]修复修补扩展权限[MagiskPolicy] Fix patching extended permissions
--[MagiskPolicy]支持更多扩展权限语法[MagiskPolicy] Support more syntax for extended permissions
--[MagiskPolicy]支持打印已加载的sepolicy规则[MagiskPolicy] Support printing out the loaded sepolicy rules
--[App]支持从ROM压缩包修补启动映像[App] Support patching boot image from ROM zips
--[应用程序]使用'init_boot.img'修补三星固件时，正确保留'boot.img'[App] Properly preserve `boot.img` when patching Samsung firmware with `init_boot.img`

### v26.1

--[App]修复撤销root权限时崩溃[App] Fix crashing when revoking root permissions
--[MagiskInit]选择预初始分区时，始终首选“ext4”分区而不是“f2fs”分区[MagiskInit] Always prefer `ext4` partitions over `f2fs` when selecting the pre-init partition
--[General]从镜像还原模块文件的上下文/所有者/组。这是v26.0中引入的回归[General] Restore module files' context/owner/group from mirror. This is a regression introduced in v26.0

### v26.0

--[常规]将最低支持的Android版本缓冲到Android6.0[General] Bump minimum supported Android version to Android 6.0
--[General]新的魔术挂载后端，支持将模块加载到系统中，并注入“overlayfs”文件[General] New magic mount backend. It supports loading modules into system with `overlayfs` files injected
